...
这一结果直接由**定理 1** 给出（结合了来自[^39]的定理 3）。

**定理 3. (Hall-Rado [^39])** 考虑一个具有 $2M$ 个节点的偶数图，使得对于每个 $k\in\{1,\cdots,M\}$ 和 $a_1,\cdots,a_M$ 的每个子序列 $a_{j_1}, \cdots, a_{j_k}$，$b_1,\cdots,b_M$ 至少 $k$ 个元素和它们连接，那么存在一个排列 $\pi$ 使得对于每个 $j$ 都有 $a_j$ 连接到 $b_{\pi(j)}$。
